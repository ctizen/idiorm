...
class QueryBuilderTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        ORM::forTable('widget')->whereIdIn(array(4, 5))->findMany();
    ...
    public function testWhereAnyIs()
    {
        ORM::forTable('widget')->whereAnyIs(array(
            ...
            array('name' => 'Fred', 'age' => 20)))->findMany();
    ...
    public function testWhereAnyIsOverrideOneColumn()
    {
        ORM::forTable('widget')->whereAnyIs(array(
            array('name' => 'Joe', 'age' => 10),
            array('name' => 'Fred', 'age' => 20)), array('age' => '>'))->findMany();
        $expected = "SELECT * FROM `widget` WHERE (( `name` = 'Joe' AND `age` > '10' ) OR ( `name` = 'Fred' AND `age` > '20' ))";
        $this->assertEquals($expected, ORM::getLastQuery());
    }
    ...
    public function testWhereAnyIsOverrideAllOperators()
    {
        ORM::forTable('widget')->whereAnyIs(array(
            ...
            array('score' => '15', 'age' => 20)), '>')->findMany();
    ...
    {
        ORM::forTable('widget')->groupBy('name')->havingLt('age', 10)->having_gt('age', 5)->findMany();
    ...
    {
        ORM::forTable('widget')->groupBy('name')->havingLte('age', 10)->having_gte('age', 5)->findMany();
        $expected = "SELECT * FROM `widget` GROUP BY `name` HAVING `age` <= '10' AND `age` >= '5'";
        $this->assertEquals($expected, ORM::getLastQuery());
    }
    ...
    {
        ORM::forTable('widget')->whereLt('age', 10)->where_gt('age', 5)->findMany();
    ...
    {
        ORM::forTable('widget')->whereLte('age', 10)->where_gte('age', 5)->findMany();
        $expected = "SELECT * FROM `widget` WHERE `age` <= '10' AND `age` >= '5'";
        $this->assertEquals($expected, ORM::getLastQuery());
    }
    ...
    {
        ORM::forTable('widget')->tableAlias('w')->join('widget_handle', array('wh.widget_id', '=', 'w.id'), 'wh')->where_equal('id', 1)->findMany();
    ...
    {
        ORM::forTable('widget')->where_equal('age', 10)->delete_many();
    ...
    {
        $record = ORM::forTable('widget')->useIdColumn(array('id1', 'id2'))->create();
        $expected = "`id1`, `id2`";
        $this->assertEquals($expected, $record->_quoteIdentifier($record->_getIdColumnName()));
    }
    ...
    {
        $record = ORM::forTable('widget')->useIdColumn(array('id1', 'id2'));
    ...
    {
        $record = ORM::forTable('widget')->useIdColumn(array('id1', 'id2'));
        $record->whereIdIn(array(
            array('id1' => 10, 'name' => 'Joe', 'id2' => 20),
            array('id1' => 20, 'name' => 'Joe', 'id2' => 30)))->findMany();
    ...
    public function testIssue74EscapingQuoteMarksInQuoteIdentifierPart()
    {
        $widget = ORM::forTable('widget')->findOne(1);
